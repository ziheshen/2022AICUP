...
if __name__ == "__main__":
    ...
    data_path = '/root/Datasets/train'
    split_ratio = [0.9,0.1,0] # train/valid/test

    image_size = 384
    save_image_path = '/root/M11115Q13/dataset/image_size%d' % image_size
    save_file_path = '/root/M11115Q13/dataset'
    ...
    for folder_name in tqdm.tqdm(os.listdir(data_path)):
        if not os.path.isdir(os.path.join(data_path,folder_name)):      #不是資料夾就不需要讀取
            continue
